...
4. **Позиция ответа критична (Метрика PAWC):**  
   ...
   Поскольку вес извлечения предложений экспоненциально затухает ($\sim 2.7\times$ при базовом $\alpha = 1.0$ и до $5\times$ при жестком штрафе), вынос прямого ответа в лид-абзац выступает практической редакционной эвристикой, максимизирующей шанс попадания главного факта в открывающие предложения ($\text{pos}(s)=0$) ответа нейросети.

> **Примечание по методологии:** Скоринг 0–100 в режиме аудита представляет собой **экспертную качественную эвристическую рубрику**, основанную на критериях Принстона KDD 2024 и практиках чанкования RAG. Для детерминированных измерений Core Web Vitals и сети комбинируйте скилл с утилитами командной строки (`lighthouse-cli`, `curl -I`).
